  ...
  return (
    <div className="container mx-auto flex flex-col py-10 px-5 md:px-0 gap-5">
      <header className="flex justify-between">
        <MyDropdowns
          name={"Categories"}
          //set can check if there have a duplicate
          category={[
        ...
        {allProducts.map((pro) => {
          return (
            ...
            >
              ...
              <p className="text-2xl font-extrabold">{pro.name}</p>
              <p>{pro.description}</p>
             <div className="w-full flex justify-between items-center">
               <div className="w-full">
                <p>$ {pro.price}</p>
              <p>Stocks {pro.stock}</p>
               </div>
               <div className="flex justify-center items-center gap-3 w-full">
                <Button variant={"ghost"} className="flex-1">See More</Button>
                <Button variant={"productBtn"} className="flex-1">Cart <ShoppingCart/></Button>
               </div>
             </div>
            </div>
          );
